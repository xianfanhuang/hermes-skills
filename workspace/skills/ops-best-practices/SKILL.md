...
| 类型 | 调度格式 | 说明 |
|------|----------|------|
| 系统维护 | `0 */6 * * *` | 24/7运行，不受市场影响 |
| 港股盘前 | `30 8 * * 1-5` | 周一到周五 |
| 港股盘中 | `30 9,10,11,13,14,15 * * 1-5` | 交易时段 |
| 美股盘前 | `30 20 * * 1-5` | 北京时间20:30 |
| 美股收盘 | `15 5 * * 2-6` | 北京时间05:15，Tue-Sat |
| 实时监控 | `every N min` | **必须加市场时段判断** |

### 实时监控cron的payload必须包含

```
如果是非交易时段（周末/假日/盘前盘后），回复NO_REPLY，不执行任何操作。
```

### 禁止

- ❌ 创建不限时段的实时监控cron（每N分钟/每N秒）
- ❌ 市场任务不限工作日
- ❌ 不检查就创建（先查现有cron列表）
